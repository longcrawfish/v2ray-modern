...
echo "检查日志目录:"
echo "  ls -la ${LOG_DIR}"
echo "测试 ws-tls 内置订阅地址:"
echo "  curl -I https://${SUBSCRIPTION_HOST}/sub/ws-tls/clash.yaml"
echo
echo "[INFO] 常见问题定位建议"
...
echo "3. 若 Clash 订阅无法访问，先确认 SUBSCRIPTION_HOST 与当前 Caddy 站点一致，且 /sub/* 已挂载 data/exports。"
echo "4. 若 reality 无法连接，先确认 REALITY_SERVER_NAME、REALITY_PUBLIC_KEY、REALITY_SHORT_ID、FLOW 与导出文件一致。"
echo "5. 若容器未启动，先运行 bash scripts/show-config.sh 检查渲染后的 Xray/Caddy 配置。"
